...
import { hasPositionFields, normalizeReading } from "@/lib/normalizeReading";
  ...
  useEffect(() => {
    ...
    return onValue(dbRef(rtdb), (snap) => {
      ...
      for (const [poleId, fields] of Object.entries(val)) {
        if (!fields || typeof fields !== "object" || !hasPositionFields(fields)) continue;

        const devTime = parseReadingTime(fields);
        const { x_mm, y_mm, htl_mm } = normalizeReading(fields);
        const { temp_c, voltage_v, solar_v, x_status, y_status } = fields;
        const signature = JSON.stringify([x_mm, y_mm, htl_mm, temp_c, voltage_v, solar_v, x_status, y_status, devTime, fields.time]);

        if (lastSignature.current[poleId] === signature) continue;
        lastSignature.current[poleId] = signature;

        // Firestore rejects undefined, so only include fields the device actually sent.
        const readingFields = { x_mm, y_mm, temp_c, voltage_v, x_status, y_status };
        if (typeof htl_mm === "number") readingFields.htl_mm = htl_mm;
